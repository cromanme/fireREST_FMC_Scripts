...
Connects to a Cisco FMC and retrieves the list of all registered devices,
...
OUTPUT_FILE: str = "../Responses/devices.json"
...
def main() -> None:
    """Retrieve all registered devices from FMC and save to JSON."""
    credentials = utils.prompt_fmc_credentials()
    fmc = utils.fmc_connect(*credentials)

    try:
        response = fmc.device.devicerecord.get()
    except Exception:
        logger.exception("Failed to retrieve devices from FMC.")
        raise SystemExit(1)

    if not response:
        logger.warning("No devices found in FMC.")
        raise SystemExit(0)
