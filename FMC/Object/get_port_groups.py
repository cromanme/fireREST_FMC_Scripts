...
Connects to a Cisco FMC and retrieves all Port Group objects, saving the
...
OUTPUT_FILE: str = "../Responses/object_port_groups.json"
...
def main() -> None:
    """Retrieve all Port Group objects from FMC and save to JSON."""
    credentials = utils.prompt_fmc_credentials()
    fmc = utils.fmc_connect(*credentials)

    try:
        response = fmc.object.portobjectgroup.get()
    except Exception:
        logger.exception("Failed to retrieve Port Group objects from FMC.")
        raise SystemExit(1)

    if not response:
        logger.warning("No Port Group objects found in FMC.")
        raise SystemExit(0)
