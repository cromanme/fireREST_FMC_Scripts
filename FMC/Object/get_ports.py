...
Connects to a Cisco FMC and retrieves all Port objects, saving the
...
OUTPUT_FILE: str = "../Responses/object_ports.json"
...
def main() -> None:
    ...
    try:
        response = fmc.object.port.get()
    except Exception:
        logger.exception("Failed to retrieve Port objects from FMC.")
        raise SystemExit(1)

    if not response:
        logger.warning("No Port objects found in FMC.")
        raise SystemExit(0)
