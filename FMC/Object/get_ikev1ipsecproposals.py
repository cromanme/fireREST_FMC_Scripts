...
# -*- coding: utf-8 -*-
"""
get_ikev1ipsecproposals.py

Connects to a Cisco FMC and retrieves list of all IKEv1 IPsec proposals,
...
OUTPUT_FILE: str = "../Responses/ikev1ipsecproposals.json"
...
def main() -> None:
    """Retrieve all IKEv1 IPsec proposals from FMC and save to JSON."""
    credentials = utils.prompt_fmc_credentials()
    fmc = utils.fmc_connect(*credentials)

    try:
        response = fmc.policy.ikev1ipsecproposals.get()
    except Exception:
        logger.exception("Failed to retrieve IKEv1 IPsec proposals from FMC.")
        raise SystemExit(1)

    if not response:
        logger.warning("No IKEv1 IPsec proposals found in FMC.")
        raise SystemExit(0)
