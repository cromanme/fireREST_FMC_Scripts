...
# -*- coding: utf-8 -*-
"""
get_securityzones.py

Connects to a Cisco FMC and retrieves all Security Zone objects, saving
...
def main() -> None:
    """Retrieve all Security Zone objects from FMC and save to JSON."""
    credentials = utils.prompt_fmc_credentials()
    fmc = utils.fmc_connect(*credentials)

    try:
        response = fmc.object.securityzone.get()
    except Exception:
        logger.exception("Failed to retrieve Security Zone objects from FMC.")
        raise SystemExit(1)

    if not response:
        logger.warning("No Security Zone objects found in FMC.")
        raise SystemExit(0)
