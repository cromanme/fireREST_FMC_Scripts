...
Connects to a Cisco FMC and retrieves all managed chassis devices, saving
...
OUTPUT_FILE: str = "../Responses/chassis.json"
...
def main() -> None:
    """Retrieve all chassis managed by FMC and save to JSON."""
    credentials = utils.prompt_fmc_credentials()
    fmc = utils.fmc_connect(*credentials)

    try:
        response = fmc.chassis.get()
    except Exception:
        logger.exception("Failed to retrieve chassis from FMC.")
        raise SystemExit(1)

    if not response:
        logger.warning("No chassis found in FMC.")
        raise SystemExit(0)
