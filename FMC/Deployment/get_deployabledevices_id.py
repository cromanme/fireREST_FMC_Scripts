...
# -*- coding: utf-8 -*-
"""
get_deployabledevices_id.py

Connects to a Cisco FMC, resolves an FTD device by hostname, and retrieves
its deployable device entry, saving the result to a JSON file.
...
def main() -> None:
    """Retrieve the deployable device entry for a specific FTD and save to JSON."""
    ...
    try:
        response = fmc.deployment.deployabledevices.get(uuid=ftd_uuid)
    except Exception:
        logger.exception(
            "Failed to retrieve deployable device info for '%s'.", FTD_HOSTNAME
        )
        raise SystemExit(1)

    if not response:
        logger.warning("No deployable device entry found for '%s'.", FTD_HOSTNAME)
        raise SystemExit(0)
