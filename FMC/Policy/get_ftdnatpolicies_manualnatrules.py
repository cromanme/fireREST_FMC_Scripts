...
OUTPUT_FILE: str = "../Responses/ftdnatpolicies_manualnatrule.json"
...
def main() -> None:
    ...
    try:
        response = fmc.policy.ftdnatpolicy.manualnatrule.get(NAT_POLICY_UUID)
    except Exception:
        logger.exception("Failed to retrieve FTD Manual NAT Rules from FMC.")
        raise SystemExit(1)

    if not response:
        logger.warning("No FTD Manual NAT Rules found in FMC.")
        raise SystemExit(0)
